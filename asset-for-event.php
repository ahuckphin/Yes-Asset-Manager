	...
	<div class="w3-col l12 s12">
		...
		<div class="w3-card-4 w3-margin w3-white">
			...
			<div class="w3-container">
				...
				<h3><b>Browse and Checkout Assets for Event</b></h3>

			<b>	Please fill up particulars as you wish. </b>

				

				<form action="asset-for-event.php" method="post">
					...
					<table>
						...
						<tr>
							<td>Event Person in Charge:</td>
							<td><input type="text" name="text-pic" value=""></td>
						</tr>
						<tr>
							<td>Remarks: </td>
					...
					<input type="submit" name="checkout-event" id="checkout-event" value="Checkout selected asset(s) for event" />
					...
					if ($conn->connect_error) {
						...
						if (isset($_POST['checkout-event'])) { //checkout-event must match with name/ID from button on previous page.
							...
							foreach ($checkboxVariable as $hostname) {
								...
								$Destination = "Event";
								$TotalRemarks = "Event name: " . $Name . ". Event person in charge: " . $PIC . ". Remarks: " . $Remarks;
								...
								if ($conn->query($Sql) === TRUE) {
									...
									$Sql = "UPDATE `details` SET `Status` = 'Checked out for event' WHERE `details`.`Hostname` = '$hostname'";
									if ($conn->query($Sql) === TRUE) {

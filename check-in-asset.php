	...
	<div class="w3-col l12 s12">
		...
		<div class="w3-card-4 w3-margin w3-white">
			...
			<div class="w3-container">
				...
				<h3><b>Check-in Assets </b></h3>

				<b>	Please fill up remarks as you wish. </b>

				<form action="check-in-asset.php" method="post">
					...
					<input type="submit" name="check-in" id="check-in" value="Check-in selected asset(s)" />

</br>
</br>
					<b> Only asset(s) available for check in is shown below. </b>
					...
					if ($conn->connect_error) {
						...
						if (isset($_POST['check-in'])) { //checkout-event must match with name/ID from button on previous page.
							...
							foreach ($checkboxVariable as $hostname) {
								...
								$Sql = "INSERT INTO MovementLog (Hostname,Destination,Remarks) VALUES ('$hostname','$Destination','$Remarks' )";
								if ($conn->query($Sql) === TRUE) {
									...
									$Sql = "UPDATE `details` SET `Status` = 'Ready for Checkout' WHERE `details`.`Hostname` = '$hostname'";
									if ($conn->query($Sql) === TRUE) {
						...
						$sql = "SELECT * FROM details WHERE Status <> 'Ready for Checkout' AND Status <> 'Checked out for permanent loan' ORDER BY Hostname ASC	 "; //<> means not equal. 
